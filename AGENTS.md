...
- Scope policy for ABI-spec-generated redirect stubs (`speet-abi-codegen`): **codegen the full libc + libSystem baseline** from BridgeSupport inputs — compatibility should rest on what is actually wired/implemented, not on which POSIX symbol gets imported next. Emit the **checked-in** subset (`-s` / `STUB_SYMBOLS`, under `speet-abi-stubs/src/generated/`) only for symbols the runtime currently intercepts; emit the **remainder** (and optionally the full baseline in one shot via `--all-symbols`) to `speet-abi-stubs/.generated/` (gitignored). Do **not** check in generated stubs for **non-baseline** OS surfaces — Apple frameworks beyond libSystem, GPLed SDK drops, and other licensed third-party headers stay out of the repo for licensing reasons, not because libc/libSystem is too large. Bulk-generating libc/libSystem locally is encouraged; checking in non-baseline framework stubs is not.
