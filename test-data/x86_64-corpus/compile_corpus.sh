#!/usr/bin/env bash
# Rebuild the pre-assembled x86_64 corpus ELF objects from their `.s` sources.
#
# Each `.s` file is assembled with `llvm-mc --triple=x86_64-unknown-elf` into a
...
ok=0; fail=0
tmp=$(mktemp /tmp/speet_x86_64_XXXXXX.o)
trap 'rm -f "$tmp"' EXIT

for src in *.s; do
    out="${src%.s}.elf"
    printf "  %-45s" "$src"
    if "$LLVM_MC" --triple=x86_64-unknown-elf --filetype=obj -o "$tmp" "$src" \
