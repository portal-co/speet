...
use yecta::{EscapeTag, TypeIdx, TagIdx};

#[test]
fn test_speculative_calls_disabled_by_default() {
    let recompiler: X86Recompiler<(), core::convert::Infallible, wasm_encoder::Function> = X86Recompiler::new();
    assert!(!recompiler.is_speculative_calls_enabled());
}

#[test]
fn test_speculative_calls_toggle() {
    let mut recompiler: X86Recompiler<(), core::convert::Infallible, wasm_encoder::Function> = X86Recompiler::new();
