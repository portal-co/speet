...
fn test_hint_tracking_with_rv32im_multiply() {
    ...
    while offset < text_data.len() {
        ...
        match Inst::decode(inst_word, Xlen::Rv32) {
            Ok((inst, is_compressed)) => {
                ...
                if let Err(_) = recompiler.translate_instruction(&inst, pc, is_compressed) {
                    break;
                }
    ...
    if !hints.is_empty() {
        ...
        for hint in hints {
            assert!(hint.value > 0, "HINT values should be positive test case numbers");
            assert!(hint.value < 100, "HINT values should be reasonable test case numbers");
        }
    } else {
...
fn test_hint_tracking_performance() {
    ...
    let mut recompiler_no_hints = RiscVRecompiler::new_with_base_pc(0x1000);
    let mut recompiler_with_hints = RiscVRecompiler::new_with_full_config(
        Pool {
            table: TableIdx(0),
            ty: TypeIdx(0),
        },
    ...
    for (i, inst) in instructions.iter().enumerate() {
        ...
        assert!(recompiler_no_hints
            .translate_instruction(inst, pc, rv_asm::IsCompressed::No)
            .is_ok());
        assert!(recompiler_with_hints
            .translate_instruction(inst, pc, rv_asm::IsCompressed::No)
            .is_ok());
    }
