...
fn test_elf_recompilation(elf_path: &Path) -> Result<usize, Box<dyn std::error::Error>> {
    ...
    let mut recompiler = RiscVRecompiler::new_with_base_pc(start_addr as u32);
    
    // Translate the entire .text section
    let bytes_translated = recompiler.translate_bytes(&text_data, start_addr as u32, Xlen::Rv32)
...
fn test_rv32i_integer_computational() {
    ...
    match test_elf_recompilation(&corpus_path) {
        Ok(bytes) => {
            println!("Successfully translated {} bytes from {}", bytes, corpus_path.display());
...
fn test_rv32im_mul_div() {
    let corpus_path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../test-data/rv-corpus/rv32im");
        ...
        for entry in entries.flatten() {
            ...
            if path.is_file() && !path.extension().map_or(false, |e| e == "s") {
                match test_elf_recompilation(&path) {
                    Ok(bytes) => {
                        println!("Successfully translated {} bytes from {}", bytes, path.display());
                        assert!(bytes > 0, "Should have translated some bytes");
...
fn test_rv32f_float() {
    let corpus_path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../test-data/rv-corpus/rv32f");
...
fn test_detailed_instruction_translation() {
    ...
    match extract_text_section(&corpus_path) {
        Ok((text_data, start_addr)) => {
            ...
            while offset < text_data.len() && offset < 100 { // Limit to first 100 bytes
                ...
                match Inst::decode(inst_word, Xlen::Rv32) {
                    Ok((inst, is_compressed)) => {
                        ...
                        match recompiler.translate_instruction(&inst, pc, is_compressed) {
                            Ok(()) => {
                                instruction_count += 1;
            ...
            assert!(instruction_count > 0, "Should have translated at least some instructions");
        }
        Err(e) => {
