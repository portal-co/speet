//! Compile [`speet-darwin-wasi-guest`] to WASM, lower host-mem imports, embed.
...
fn main() {
    ...
    let guest_src = workspace_root.join("crates/os/speet-darwin-wasi-guest/src");
    ...
    if try_build_and_lower(&workspace_root, &canonical) {
        ...
        panic!(
            "speet-darwin-wasi-guest build failed and no fallback at {}",
            fallback.display()
        );
    }
}
...
fn guest_wasm_path(workspace_root: &Path, target: &str) -> PathBuf {
    if let Ok(dir) = std::env::var("CARGO_TARGET_DIR") {
        let p = PathBuf::from(dir)
            .join(target)
            .join("release")
            .join("speet_darwin_wasi_guest.wasm");
        if p.is_file() {
            return p;
        }
    }
