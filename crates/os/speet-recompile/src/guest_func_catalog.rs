...
impl GuestFuncCatalog {
    ...
    pub fn from_layout(
        ...
        let mut entries =
            Vec::with_capacity(n_translated_slots as usize + n_redirect_shims as usize + 1);
    ...
    pub fn from_wasm(
        ...
        let n_translated_slots = n_code.saturating_sub(1 + n_redirect_shims).saturating_sub(
            if crate::drive::has_data_init_export(wasm) {
                1
            } else {
                0
            },
    ...
    pub fn entry_func(&self, entry_func_idx: u32) -> Option<&GuestFuncEntry> {
        self.entries
            .iter()
            .find(|e| e.local_func_idx == entry_func_idx && !e.is_halt)
    }
...
mod tests {
    ...
    fn catalog_maps_halt_and_entry() {
        ...
        let catalog =
            GuestFuncCatalog::from_translated(&translated, 0x1000, BinArch::X86_64, 5, 12, 0);
