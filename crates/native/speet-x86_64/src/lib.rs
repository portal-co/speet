...
impl<Context, E> X86Recompiler<Context, E> {
    ...
    pub fn setup_traps<F>(
        ...
        *rctx.layout_mut() = LocalLayout::empty();
        ...
        // `declare_params` and appends a genuine per-function *local*, not a
        // wasm function *param* — calling it before the mark folded this
        // scratch into the param list, corrupting every function's
...
pub fn plt_calling_convention(symbol: &str) -> speet_plugin_api::external_target::CallingConvention {
    ...
    speet_abi_stubs::plt_calling_convention(&ImportManifest::integrated_native(), BinArch::X86_64, symbol)
}
