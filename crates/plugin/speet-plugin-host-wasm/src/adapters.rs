//! The six `Wasm*Plugin` adapters: each implements one `speet-plugin-api`
//! trait by encoding a `remote::XRequest`, driving it through
//! [`WasmPlugin::call_raw`], and decoding the `remote::XResponse`.
//!
...
//! failure (a WASM trap, a malformed response) through. Such a failure
//! means the guest is fundamentally broken, so these adapters `expect`/
//! `panic!` rather than inventing a fake zero value — consistent with
...
fn roundtrip<Req: WireEncode, Resp: WireDecode>(engine: &WasmPlugin, req: Req) -> Resp {
    let mut payload = Vec::new();
    req.encode(&mut payload);
    let resp_bytes = engine
        .call_raw(&payload)
        .expect("wasm plugin call failed (transport-level)");
    let (resp, _) = Resp::decode(&resp_bytes).expect("wasm plugin returned malformed response");
    resp
}

// ── Arch ─────────────────────────────────────────────────────────────────

pub struct WasmArchPlugin {
    engine: WasmPlugin,
}
impl WasmArchPlugin {
    pub fn new(engine: WasmPlugin) -> Self {
        Self { engine }
    }
}
impl ArchPlugin for WasmArchPlugin {
    ...
    fn bind_imports(&self, _imports: &dyn HostImports) {
        // No-op: the WASM realization of §2.7 binds imports at
        // instantiation time (the `host.speet_host_call` import is wired
        // into the `Linker` in `WasmPlugin::load`, before this method could
        // ever run), not via a separate post-load call. See `crate::abi`.
    }
}

// ── AddressMapper ────────────────────────────────────────────────────────

pub struct WasmAddressMapperPlugin {
    engine: WasmPlugin,
}
impl WasmAddressMapperPlugin {
    pub fn new(engine: WasmPlugin) -> Self {
        Self { engine }
    }
}
impl AddressMapperPlugin for WasmAddressMapperPlugin {
...
pub struct WasmMemoryAccessPlugin {
    engine: WasmPlugin,
}
impl WasmMemoryAccessPlugin {
    pub fn new(engine: WasmPlugin) -> Self {
        Self { engine }
    }
}
impl MemoryAccessPlugin for WasmMemoryAccessPlugin {
...
pub struct WasmTablePlugin {
    engine: WasmPlugin,
}
impl WasmTablePlugin {
    pub fn new(engine: WasmPlugin) -> Self {
        Self { engine }
    }
}
impl TablePlugin for WasmTablePlugin {
    fn indirect_jump(
        &self,
        target_local: u32,
...
pub struct WasmObjectModelPlugin {
    engine: WasmPlugin,
}
impl WasmObjectModelPlugin {
    pub fn new(engine: WasmPlugin) -> Self {
        Self { engine }
    }
}
impl ObjectModelPlugin for WasmObjectModelPlugin {
...
fn snippet_call(engine: &WasmPlugin, req: ObjectModelRequest) -> speet_plugin_api::PResult<CodeSnippet> {
...
impl WasmTargetPlugin {
    pub fn new(engine: WasmPlugin) -> Self {
        Self { engine }
    }
}
impl TargetPlugin for WasmTargetPlugin {
