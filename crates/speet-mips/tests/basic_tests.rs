...
fn test_branch_instructions() {
    ...
    let mut ctx = ();
    recompiler.translate_instruction(&mut ctx, &bne_instruction, &mut |locals| {
        Function::new(locals.collect::<Vec<_>>())
    }).unwrap();
    ...
    let mut ctx = ();
    recompiler.translate_instruction(&mut ctx, &blez_instruction, &mut |locals| {
        Function::new(locals.collect::<Vec<_>>())
    }).unwrap();
    ...
    let mut ctx = ();
    recompiler.translate_instruction(&mut ctx, &bgtz_instruction, &mut |locals| {
        Function::new(locals.collect::<Vec<_>>())
    }).unwrap();
    ...
    let mut ctx = ();
    recompiler.translate_instruction(&mut ctx, &bltz_instruction, &mut |locals| {
        Function::new(locals.collect::<Vec<_>>())
    }).unwrap();
    ...
    let mut ctx = ();
    recompiler.translate_instruction(&mut ctx, &bgez_instruction, &mut |locals| {
        Function::new(locals.collect::<Vec<_>>())
    }).unwrap();
}
...
fn test_syscall_callback() {
    ...
    let mut syscall_callback = |_: &SyscallInfo, _ctx: &mut (), _: &mut CallbackContext<'_, (), core::convert::Infallible, wasm_encoder::Function>| {
        syscall_called = true;
    };
...
fn test_break_callback() {
    ...
    let mut break_callback = |_: &BreakInfo, _ctx: &mut (), _: &mut CallbackContext<'_, (), core::convert::Infallible, wasm_encoder::Function>| {
        break_called = true;
    };
    ...
    let mut ctx = ();
    recompiler.translate_instruction(&mut ctx, &break_instruction, &mut |locals| {
        Function::new(locals.collect::<Vec<_>>())
    }).unwrap();
...
fn test_multiplication_division() {
    ...
    let mut ctx = ();
    recompiler.translate_instruction(&mut ctx, &mult_instruction, &mut |locals| {
        Function::new(locals.collect::<Vec<_>>())
    }).unwrap();
    ...
    let mut ctx = ();
    recompiler.translate_instruction(&mut ctx, &div_instruction, &mut |locals| {
        Function::new(locals.collect::<Vec<_>>())
    }).unwrap();
}
...
fn test_hi_lo_operations() {
    ...
    let mut ctx = ();
    recompiler.translate_instruction(&mut ctx, &mfhi_instruction, &mut |locals| {
        Function::new(locals.collect::<Vec<_>>())
    }).unwrap();
    ...
    let mut ctx = ();
    recompiler.translate_instruction(&mut ctx, &mflo_instruction, &mut |locals| {
        Function::new(locals.collect::<Vec<_>>())
    }).unwrap();
}