...
fn test_simple_syscall() {
    ...
    let mut syscall_callback = |_: &speet_mips::SyscallInfo, _ctx: &mut (), _: &mut speet_mips::CallbackContext<'_, (), core::convert::Infallible, wasm_encoder::Function>| {
        syscall_called = true;
    };
...
fn test_simple_jr() {
    ...
    recompiler.translate_instruction(&mut ctx, &jr_instruction, &mut |locals| {
        Function::new(locals.collect::<Vec<_>>())
    }).unwrap();
}
