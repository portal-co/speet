...
fn test_load_store_variants() {
    ...
    recompiler.translate_instruction(&mut ctx, &lb, &mut |locals| Function::new(locals.collect::<Vec<_>>())).unwrap();

    // LBU: lbu $t0, 1($t1) -> opcode 0x912A0001
    let lbu = Instruction::new(0x912A0001, 0x2000, InstrCategory::CPU);
    recompiler.translate_instruction(&mut ctx, &lbu, &mut |locals| Function::new(locals.collect::<Vec<_>>())).unwrap();

    // LH: lh $t0, 2($t1) -> opcode 0x852A0002
    let lh = Instruction::new(0x852A0002, 0x2000, InstrCategory::CPU);
    recompiler.translate_instruction(&mut ctx, &lh, &mut |locals| Function::new(locals.collect::<Vec<_>>())).unwrap();

    // LHU: lhu $t0, 2($t1) -> opcode 0x952A0002
    let lhu = Instruction::new(0x952A0002, 0x2000, InstrCategory::CPU);
    recompiler.translate_instruction(&mut ctx, &lhu, &mut |locals| Function::new(locals.collect::<Vec<_>>())).unwrap();

    // SB: sb $t0, 3($t1) -> opcode 0xA12A0003
    let sb = Instruction::new(0xA12A0003, 0x2000, InstrCategory::CPU);
    recompiler.translate_instruction(&mut ctx, &sb, &mut |locals| Function::new(locals.collect::<Vec<_>>())).unwrap();

    // SH: sh $t0, 4($t1) -> opcode 0xA52A0004
    let sh = Instruction::new(0xA52A0004, 0x2000, InstrCategory::CPU);
    recompiler.translate_instruction(&mut ctx, &sh, &mut |locals| Function::new(locals.collect::<Vec<_>>())).unwrap();
    ...
    recompiler.translate_instruction(&mut ctx, &lw, &mut |locals| Function::new(locals.collect::<Vec<_>>())).unwrap();

    let sw = Instruction::new(0xAD2A0004, 0x2000, InstrCategory::CPU);
    recompiler.translate_instruction(&mut ctx, &sw, &mut |locals| Function::new(locals.collect::<Vec<_>>())).unwrap();
}
