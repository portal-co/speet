...
- **`b7e50f4`** ("runtime 1.2.4") generalized PLT redirection: `ExternalTargetTable`/`PltHookTable` keyed by `(LibraryId, address)`, `PltHookTarget::WasmImport` vs `Ambient`, and PC-check hooks in `speet-x86_64`/`speet-aarch64` — replacing per-arch `plt_by_addr` + `plt_imports` pairs (principles 1–2).
- **`ccae746`** ("runtime 1.2.5") centralized `speet_abi_stubs::plt_calling_convention(manifest, …)` and manifest-driven PLT tests; restored corpus load-addr contract, start_addr-aware WASM cache keys, and syscall dispatcher per-arm continuation.
- **`6246881`** ("runtime 1.2.6") added ABI-codegen `registry.rs` dispatch for checked-in redirect stubs (`write`/`exit`); native `redirect` emitters exist but are not yet wired into PC-check hook emission.
